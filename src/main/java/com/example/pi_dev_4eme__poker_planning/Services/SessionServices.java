...
public class SessionServices implements ISessionRepositories {
    ...
    public List<User> getDeveloperUsersForSession(Integer idSession) {
        //Session session = sessionRepositories.findById(idSession).orElse(null);
        Session session = sessionRepositories.findByIdSession(idSession).orElse(null);
        if(session != null ){
            List<User> alluser = session.getUsers();
            return alluser.stream().filter(user -> user.getRolee() ==  Role.developer).collect(Collectors.toList());
        }else {
            return null;
        }
